...
import { X, MessageSquareText, Copy, Check, Send, Sparkles, Smartphone, ShieldCheck } from 'lucide-react';
...
export function WhatsAppReminderModal({
  ...
  // Realistic Indian B2B collection templates
  const templates = {
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full animate-in fade-in zoom-in-95 duration-150 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-emerald-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold">WhatsApp AI Follow-up</h3>
              <p className="text-xs text-emerald-100 flex items-center gap-1">
                <Sparkles className="w-3 h-3" />
                Auto-formatted invoice & UPI payment link
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="relative p-4 rounded-xl bg-emerald-50/60 border border-emerald-200 text-xs text-slate-800 whitespace-pre-wrap font-sans leading-relaxed shadow-2xs">
            <div className="absolute top-2 right-2">
              <button
                onClick={handleCopy}
                className="p-1.5 bg-white border border-emerald-200 rounded-md text-slate-600 hover:text-slate-900 shadow-2xs transition"
                title="Copy Text"
              >
          ...
          <div className="flex items-center gap-2 text-[11px] text-slate-400 bg-slate-50 p-2 rounded-lg border border-slate-200/60">
            <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
            <span>Sends verified WhatsApp message via official Whatsbill Business API webhook.</span>
          </div>
        </div>
