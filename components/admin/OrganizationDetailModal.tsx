      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden text-slate-100">
        ...
        {isLoading ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-slate-400">
            ...
            <p className="text-sm">Retrieving real-time organization profile...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Sub-tab Navigation */}
            <div className="flex items-center gap-2 px-6 pt-3 border-b border-slate-800 bg-slate-950/30 overflow-x-auto text-xs">
              <button
                onClick={() => setActiveSubTab('summary')}
                className={`pb-2.5 font-semibold px-2 border-b-2 transition ${
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded-xl bg-slate-950/40 border border-slate-800 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div className="flex items-center gap-2 p-2.5 rounded-lg bg-slate-900 border border-slate-800/80">
                        {org?.gstin ? (
                      ...
                      {data?.invoices.map((inv: any) => (
                        <div key={inv.id} className="p-3 bg-slate-950/40 flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-slate-400">{formatDate(inv.issue_date, 'short')} • Status: <span className="capitalize text-amber-300">{inv.status}</span></p>
                          </div>
                  ...
                  {data?.payments.length === 0 ? (
                    ...
                    <div className="divide-y divide-slate-800 border border-slate-800 rounded-xl overflow-hidden text-xs">
                      {data?.payments.map((p: any) => (
                        <div key={p.id} className="p-3 bg-slate-950/40 flex items-center justify-between">
                          <div>
                            <p className="font-bold text-white uppercase">{p.method} Payment</p>
                    ...
                    <div className="divide-y divide-slate-800 border border-slate-800 rounded-xl overflow-hidden text-xs">
                      {data?.messageLogs.map((m: any) => (
                        <div key={m.id} className="p-3 bg-slate-950/40 flex items-center justify-between">
                          <div>
                            <p className="font-bold text-white">{m.direction === 'inbound' ? 'Inbound Message' : 'Outbound Dispatch'}</p>
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between gap-3">
              {onDelete && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-semibold transition"
                >
              ...
              <button
                onClick={onClose}
                className="ml-auto px-4 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition"
              >
                Close Details
              </button>
            </div>
